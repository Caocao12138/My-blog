...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
    ...
    const data = {
        ...
        [`rows|${query.limit || 10}`]: [
            {
                id: "@guid", // @gu函数随机生成id,保证id的唯一性
                title: "@ctitle(4,40)", //@c函数随机生成中文的标题
                description: "@cparagraph(1,10)", //随机生成中文段落,1-10条
                category: { //分类
                    "id|1-10": 0,
                    name: "分类@id",
                },
                "scanNumber|0-3000": 0, //浏览量 0-3000
                "commentNumber|0-300": 0, //评论数量 0-300
                "thumb|1": ["@image(240x180, @color, #fff,@ctitle)"], // 缩略图 随机生成 // Mock.Random.image(尺寸,背景颜色,前景色,文字)
                createDate: `@date('T')`, // 博客发布时间 : 时间戳  // @date('T') 随机时间('日期格式')
            }
        ],
    }
    return Mock.mock({
...
Mock.mock(/^\/api\/blog\/[^/]+/, "get", {
    ...
    data: {
        ...
        createDate: "@date('T')",
        "thumb|1": [Mock.Random.image("240x180", "#000", "#fff", "Random image"), null], // 缩略图 随机生成 // Mock.Random.image(尺寸,背景颜色,前景色,文字)
        toc: [
